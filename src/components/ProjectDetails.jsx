  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center w-full h-full overflow-hidden backdrop-blur-sm'>
      ...
      <div className='flex items-center justify-between mt-4'>
        <div className='flex gap-3'>{tags.map((tag)=>(
          <img key={tag.id} src={tag.path} alt={tag.name} className='rounded-lg size-10 hover-animation'/>
        ))}</div>
        <a className='inline-flex items-center gap-1 font-medium hover-animation cursor-pointer' target="_blank" href={href}>View Project <img src='assets/arrow-up.svg' className='size-4'/> </a>
      </div>
      </div>
        </motion.div> </div>
  )
